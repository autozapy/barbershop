...
function adicionarAgendamento(stepContent) {
  ...
  const dadosAgendamento = {
    ...
    dataHoraInicio: bookingData.date + 'T' + bookingData.time
  };
...
function inicializarHome() {
  ...
  // Mostra mensagem de carregando enquanto espera resposta
  $('#hero-section h1').text('Carregando...');
  $('#hero-section p.lead').html(`Estamos carregando as informações da barbearia. Aguarde um momento...`);
  $('#start-booking-btn, #check-appointments-btn, .destaques-estabelecimento, #barbearia-info').hide();
  ...
  $.get(url)
    .done(function (data) {
      ...
      document.title = data.nome + " - Agendamento Online";

      $('#barbearia-nome').text(data.nome);
      $('#barbearia-descricao').html(data.descricao);
      $('#barbearia-endereco').text(data.endereco);
      $('#barbearia-contato').text(data.telefone);
      $('#horario-semana').text(data.horarioSemana);
      $('#horario-sabado').text(data.horarioSabado);
      $('#horario-domingo').text(data.horarioDomingo);
      $('#rota-maps').attr('href', data.mapsUrl);

      const destaques = (data.destaques || []).map(d => `
        <div class="col-md-4 mb-2">
          <div class="d-flex flex-column align-items-center">
            <i class="bi bi-${d.icone} barber-gold mb-2" style="font-size: 2rem;"></i>
            <h5 class="fw-semibold">${d.titulo}</h5>
            <p class="text-light small mb-0">${d.descricao}</p>
          </div>
        </div>
      `).join('');

      $('#barbearia-destaques').html(destaques);

      // Exibe se tudo correu bem
      $('#start-booking-btn, #check-appointments-btn, .destaques-estabelecimento, #barbearia-info').fadeIn();
...
$('#start-booking-btn-mobile').on('click', function () {
  $('#start-booking-btn').trigger('click');
});

$('#check-appointments-btn-mobile').on('click', function () {
  $('#check-appointments-btn').trigger('click');
});
...
function renderPersonalInfo($container) {
  ...
  $('#personal-info-form input').removeClass('is-invalid');
...
function initializePhoneSearch() {
  ...
  $form.off('submit').on('submit', function (e) {
    e.preventDefault();
    carregarAgendamentosCliente($phoneInput.val());
  });
